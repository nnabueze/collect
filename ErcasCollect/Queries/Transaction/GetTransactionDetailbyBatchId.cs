...
namespace ErcasCollect.Queries.BillerQuery
{
    public class GetTransactionByBatchIDQuery : IRequest<IEnumerable<ReadTransactionDto>>
    {
        ...
        //        var result = await transactionbybatchidRepository.FindAllInclude(x => x.BatchId == query.id, x => x.StatusCode, x => x.User, x => x.Biller, x => x.PaymentChannel, x => x.TransactionType);
        //        if (result != null)
        //        {
